...
class ReservoirBuffer:
    ...
    def add(self, batch_x, batch_features, batch_labels):
        assert batch_x.size(0) == batch_features.size(0) == batch_labels.size(0)

        batch_x, batch_features, batch_labels = batch_x.to(self.device), batch_features.to(self.device), batch_labels.to(self.device)
        ...
        if self.buffer.size(0) == 0:
            ...
            buffer_shape[0] = 0
            self.buffer_labels = torch.empty(buffer_shape).to(self.device)
    ...
    def sample(self, batch_size):
        ...
        return batch_x, batch_features, batch_labels, indices
    ...
    def sample_indices(self, indices):
        return self.buffer[indices], self.buffer_features[indices], self.buffer_labels[indices]