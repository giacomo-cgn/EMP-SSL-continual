...
class NCMClassifier(nn.Module):
    ...
    def __init__(self, normalize: bool = False, update_momentum: float = 0.9):
        """
    ...
    def update_class_means_dict(self, features: Tensor, labels: Tensor):
        """
        Update dictionary of class means.

        :param features: input features of shape (batch_size, feature_size)
        :param labels: class labels of shape (batch_size,)
        """
        assert features.size(0) == labels.size(0), "sizes must match"
        for feature, label in zip(features, labels):
            if label.item() not in self.class_means_dict:
                self.class_means_dict[label.item()] = feature.clone()
            else:
                self.class_means_dict[label.item()] = (
                    self.update_momentum * feature
                    + (1 - self.update_momentum) * self.class_means_dict[label.item()]
                )
