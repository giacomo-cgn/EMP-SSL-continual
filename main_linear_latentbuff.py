...
import datetime
from src.buffer import LatentReservoirBuffer
...
parser.add_argument('--dir', type=str, default="EMP-linear-latentbuff",
...
parser.add_argument('--mem_size', type=int, default=2000,
                    help='Memory size for the latent buffer (default:2000)')
parser.add_argument('--latent_mem_bs', type=int, default=100,
                    help='Memory size for the latent buffer (default:100)')
...
# Init latent buffer
latent_buffer = LatentReservoirBuffer(args.mem_size)
    ...
    for exp_idx, exp_trainset in enumerate(exps_trainset):
        ...
        for epoch in range(args.epoch):            
            for step, step_data in tqdm(enumerate(dataloader)):
                ...
                latent_buffer.add(z_pre_avg.detach(), label)

                for stp in range(args.passes_linear):
                    # sample from buffer
                    if len(latent_buffer.buffer_features) > args.latent_mem_bs:
                        mem_z_pre_avg, mem_label, _ = latent_buffer.sample(args.latent_mem_bs)
                        mem_z_pre_avg, mem_label = mem_z_pre_avg.to(device), mem_label.to(device)

                        concat_labels = torch.cat([label, mem_label], dim=0)
                        concat_z_pre_avg = torch.cat([z_pre_avg, mem_z_pre_avg], dim=0)
                    else:
                        concat_labels = label
                        concat_z_pre_avg = z_pre_avg
                    LL.zero_grad()
                    logits = LL(concat_z_pre_avg)
                    loss_linear = criterion_linear(logits, concat_labels)
                    LL.zero_grad()
                    loss_linear.backward()
                    opt_linear.step()

                # Update latent memory
                latent_buffer.add(z_pre_avg.detach(), label.detach())
